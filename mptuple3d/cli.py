...
def plot3d_vispy(
    ...
    """
    3D plot (Vispy):     reads (x,y,z[,color]) tuples from stdin via messagepack.
    """
...
def plot2d_vispy(
    ...
    """
    2D plot (Vispy):     reads (x,y[,color]) tuples from stdin via messagepack.
...
@click.option(
    "--size",
    type=float,
    help="Point size (not applicable for Polyscope)",
)
@click.option(
    "--title",
    type=str,
